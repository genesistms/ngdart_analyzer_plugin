...
/// for their design.
class OffsettingConstantEvaluator extends ast.ConstantEvaluator {
  ...
  Object? visitAdjacentStrings(ast.AdjacentStrings node) {
    ...
    for (final string in node.strings) {
      ...
      if (identical(value, ast.ConstantEvaluator.NOT_A_CONSTANT)) {
        return value;
      }
  ...
  Object? visitBinaryExpression(ast.BinaryExpression node) {
    if (node.operator.type == TokenType.PLUS) {
      ...
      if (identical(leftOperand, ast.ConstantEvaluator.NOT_A_CONSTANT)) {
        return leftOperand;
      }
      final rightOperand = node.rightOperand.accept(this);
      if (identical(rightOperand, ast.ConstantEvaluator.NOT_A_CONSTANT)) {
        return rightOperand;
      }
      // numeric or {@code null}
      if (leftOperand is String && rightOperand is String) {
        final gap = node.rightOperand.offset - node.leftOperand.offset - node.leftOperand.length;
