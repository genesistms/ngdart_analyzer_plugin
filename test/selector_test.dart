...
import 'package:ngdart_analyzer_plugin/src/selector/and_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/attribute_contains_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/attribute_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/attribute_starts_with_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/attribute_value_regex_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/class_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/contains_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/element_name_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/not_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/or_selector.dart';
import 'package:ngdart_analyzer_plugin/src/selector/parser.dart';
import 'package:test/test.dart';

void main() {
  group('Parser', () {
    test('match', () {
      final selector = Parser(
        fileOffset: 10,
        original: '[ng-for][ng-for-of]',
      ...
      expect(selector.selectors, hasLength(2));
      {
        ...
        expect(nameElement.source, NonExistingSource.unknown);
    ...
    test('attribute_beginsWithOperator_noValue', () {
      try {
        Parser(original: '[foo^=]', source: NonExistingSource.unknown).parse();
    ...
    test('attribute_containsOperator_noValue', () {
      try {
        Parser(original: '[foo*=]', source: NonExistingSource.unknown).parse();
      } on FormatException catch (e) {
        expect(e.message, contains('Expected a value after *=, got ]'));
    ...
    test('attribute_hasValue', () {
      final selector = Parser(
        fileOffset: 10,
        original: '[kind=pretty]',
        source: NonExistingSource.unknown,
      ).parse() as AttributeSelector;
      {
    ...
    test('attribute_hasValueWithQuotes', () {
      final selector = Parser(
        source: NonExistingSource.unknown,
        fileOffset: 10,
        original: '''[single='quotes'][double="quotes"]''',
      ).parse() as AndSelector;
      expect(selector.selectors, hasLength(2));
      {
        final subSelector = selector.selectors[0] as AttributeSelector;
    ...
    test('attribute_hasWildcard', () {
      final selector = Parser(
        source: NonExistingSource.unknown,
        fileOffset: 10,
        original: '[kind*=pretty]',
      ).parse() as AttributeContainsSelector;
      {
        final nameElement = selector.nameElement;
        expect(nameElement.source, NonExistingSource.unknown);
        expect(nameElement.string, 'kind');
    ...
    test('attribute_regularOperator_noValue', () {
      try {
        Parser(
          source: NonExistingSource.unknown,
          fileOffset: 0,
    ...
    test('attribute_startsWith', () {
      final selector = Parser(
        source: NonExistingSource.unknown,
        fileOffset: 10,
        original: '[foo^=bar]',
      ).parse() as AttributeStartsWithSelector;
      expect(selector.nameElement.string, 'foo');
      expect(selector.value, 'bar');
    });

    test('attribute_startsWith_quoted', () {
      final selector = Parser(
        source: NonExistingSource.unknown,
        fileOffset: 10,
        original: '[foo^="bar"]',
    ...
    test('attribute_textRegex', () {
      final selector = Parser(
        source: NonExistingSource.unknown,
        fileOffset: 10,
        original: '[*=/pretty/]',
      ).parse() as AttributeValueRegexSelector;
    ...
    test('complex_ast', () {
      final selector = Parser(
        source: NonExistingSource.unknown,
        fileOffset: 10,
        original: 'aaa, bbb:not(ccc), :not(:not(ddd)[eee], fff[ggg])',
      ).parse() as OrSelector;
      ...
      }
      {
        ...
        expect(subSelector.toString(), ":not(:not(ddd) && [eee] || fff && [ggg])");
        {
          ...
          expect(subSelector2.toString(), ":not(ddd) && [eee] || fff && [ggg]");
          {
            ...
            expect(subSelector3.toString(), ":not(ddd) && [eee]");
            {
              ...
              {
                final subSelector5 = subSelector4.condition as ElementNameSelector;
          ...
          }
          {
            ...
            {
              final subSelector4 = subSelector3.selectors[0] as ElementNameSelector;
    ...
    test('contains', () {
      final selector = Parser(
        source: NonExistingSource.unknown,
        fileOffset: 10,
        original: ':contains(/aaa/)',
      ).parse() as ContainsSelector;
